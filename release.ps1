...
# turbo_tracker.py owns the version. An explicit -Version (the tag, in CI) is
...
if (-not $Version) {
    ...
    throw ("Version mismatch: asked for $Version but turbo_tracker.py " +
           "declares $ProjectVersion. Bump __version__ and re-tag.")
}

$exePath = Join-Path $PSScriptRoot "dist\TurboTracker.exe"
$releaseDir = Join-Path $PSScriptRoot "dist\release"
$artifactName = "TurboTracker-v$Version-windows-x64"
$zipPath = Join-Path $releaseDir "$artifactName.zip"
$zipChecksumPath = "$zipPath.sha256"
$installerPath = Join-Path $releaseDir "TurboTracker-v$Version-Setup.exe"
$installerChecksumPath = "$installerPath.sha256"
$stageRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("TurboTracker-release-" + [guid]::NewGuid())
...
try {
    ...
    try {
        ...
        $required = @('TurboTracker.exe', 'portable.flag', 'README.md', 'LICENSE')
        ...
        $private = $relativePaths | Where-Object {
            ($_ -match '^(data|local|captures)[/\\]') -or
            ($_ -match '(settings\.json|turbo\.db|\.jsonl)$')
        }
        if ($private) {
